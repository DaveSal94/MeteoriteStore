...
**************************************************/
public class SpaceJunk extends Meteorites
{
	Continents cont = new Continents();
	public SpaceJunk(double newlongitude, double newlatitude, float newprice,int newIDnum,int newtype, boolean newSold) {
	...
	* Outputs: suggested price
	...
	public float pricefunction(float priceval,Position location)
	{
		...
		float currentprice =(float) 10.00 + cont.LoadFile(location) ;
		int n = JOptionPane.showConfirmDialog(null,"Would you like to update to suggested price?\n"+" "+"$"+currentprice,"Update Price", JOptionPane.YES_NO_OPTION);
